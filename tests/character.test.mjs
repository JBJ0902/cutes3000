...
const events={},store=new Map();const actor={style:{},setAttribute(){},removeAttribute(){},src:'',alt:''},reactionActor={style:{},setAttribute(){},removeAttribute(){},src:'',alt:''};
const host={dataset:{mood:'0'},isConnected:true,querySelector:s=>s==='.motion-reaction'?reactionActor:actor,removeAttribute:k=>delete host.dataset[k==='data-reaction'?'reaction':k],getBoundingClientRect:()=>({top:0,bottom:450})};
...
assert.equal(vm.runInContext('reactionSource("heart",1,"classic")',ctx),'assets/reactions/heart/school-01.webp');assert.equal(vm.runInContext('reactionSource("mistake",5,"floral")',ctx),'assets/reactions/mistake/floral-05.webp');
...
ctx.pauseCheck=()=>paused;vm.runInContext('initCharacter(pauseCheck)',ctx);assert.equal(loads.length,78);assert.equal(host.dataset.ready,'false');loads.slice(0,24).forEach(x=>x.onload());assert.equal(host.dataset.ready,'true');
...
events.click({target:{closest:s=>s==='[data-character-outfit]'?{dataset:{characterOutfit:'floral'}}:null}});assert.equal(host.dataset.ready,'false');loads.slice(24,48).forEach(x=>x.onload());assert.equal(host.dataset.ready,'true');assert(actor.src.includes('floral-'));
vm.runInContext("reactPortrait('heart')",ctx);assert.equal(host.dataset.reaction,'heart');assert(reactionActor.src.includes('assets/reactions/heart/floral-01.webp'));for(let i=1;i<60;i++)callback(32010+i*60);assert.equal(host.dataset.reaction,undefined);
...
for(const kind of ['heart','angry','mistake'])for(const outfit of ['school','floral'])for(let i=1;i<=5;i++){const p=new URL(`../dist/assets/reactions/${kind}/${outfit}-${String(i).padStart(2,'0')}.webp`,import.meta.url);assert(existsSync(p));const b=readFileSync(p);assert.equal(b.subarray(0,4).toString('hex'),'52494646');}
...
console.log('PASS: 48 base poses, 30 reaction poses, frame bounds, preload/fallback, pause/visibility, controls, persistence and module dependencies');
